...
public class CopyOfJsonUtil {
	...
	public static String object2jsonByMap(Object obj,Map<String,String> beanfeildMap) {
		...
		if (obj == null) {
			...
			json.append(bean2jsonByMap(obj,beanfeildMap));
		}
		return json.toString();
	}
	
	@SuppressWarnings("unchecked")
	public static String bean2jsonByMap(Object bean,Map<String,String> feildsMap) {
		StringBuilder json = new StringBuilder();
		json.append("{");
		...
		try {
			...
			for(int k=0;k<mothods.length;k++){
				if(mothods[k].getName().equals("readFeildMap")){
					//hasFieldMap = true;
					try{
						fieldMap = (Map<String,String>)mothods[k].invoke(bean);
					}catch(Exception e){
			...
			for (int i = 0; i < props.length; i++) {
				try {
					...
					if(fieldMap!=null && fieldMap.containsKey(filedname)){
							...
							name = object2json(zhname);
					}else{
						name = object2json(filedname);
					}
					//
